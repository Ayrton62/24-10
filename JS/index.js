...
function mostrarPersonajes(personajes) {
    ...
    personajes.forEach(personaje => {
        ...
        <input type="number" id="inputTrain" placeholder="Ingrese el numero de dias de entrenamiento">
        `
        const btnBorrar = crearBotonBorrar(personaje);
        const btnTrain = crearBotonEntrenar(personaje);
        li.appendChild(btnBorrar);
        li.appendChild(btnTrain);
        listadoPersonajes.appendChild(li);
    });
}

function crearBotonEntrenar(personaje) {
    const btnTrain = document.createElement('button');
    btnTrain.innerText = "Entrenar";
    btnTrain.addEventListener('click', () => {
        entrenarPersonaje(personaje);
    })
    return btnTrain;
}

function entrenarPersonaje(personaje) {
    let time = document.getElementById('inputTrain').value
    let newPower = personaje.poder * time 

    Swal.fire({
        title: 'Perfecto!',
        text: `El poder de tu personaje seria de ${newPower}`,
        imageUrl: `${personaje.imagen}`,
        imageAlt: `${personaje.dato}`,
    })
}
